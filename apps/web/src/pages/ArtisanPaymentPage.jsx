import React from 'react';
import { Helmet } from 'react-helmet';
import { Link } from 'react-router-dom';
import { Button } from '@/components/ui/button';

const ArtisanPaymentPage = () => {
  return (
    <div className="min-h-[70vh] flex flex-col items-center justify-center pt-24 bg-muted px-4 text-center">
      <Helmet>
        <title>Paiements | ArtisanCongo</title>
      </Helmet>
      <h1 className="text-3xl md:text-4xl font-bold mb-4">Mes Paiements</h1>
      <p className="text-muted-foreground mb-8 max-w-2xl">
        Gérez vos transactions, vos retraits et consultez l'historique de vos paiements.
      </p>
      <Button asChild>
        <Link to="/artisan-dashboard">Retour au tableau de bord</Link>
      </Button>
    </div>
  );
};

export default ArtisanPaymentPage;