...
import {
  ...
  AlertCircle, FileText, User, Phone, XCircle, Mail, Globe, Star, PlusCircle
} from 'lucide-react';
import { useAuth } from '@/contexts/AuthContext.jsx';

const ArtisanPublicProfilePage = () => {
  const { id } = useParams();
  ...
  const fetchArtisanAndReviews = async () => {
    try {
      ...
      if (record.profile_visibility !== 'visible') {
        ...
        const reviewsData = await pb.collection('ratings').getFullList({
          ...
          sort: '-created',
          expand: 'user_id', // Expand if we want user details, though user might be anonymous
          $autoCancel: false
        });
  ...
  return (
    <>
                ...
                <CardContent className="p-6 md:p-8 bg-card">
                  ...
                  <div className="flex flex-wrap gap-2 mb-4">
                    ...
                    {artisan.statut_artisan && (
                      <Badge className="badge-primary capitalize">{artisan.statut_artisan}</Badge>
                    )}
                  </div>
                  ...
                  <div className="space-y-3 mb-8">
                    ...
                    <div className="flex items-center text-muted-foreground">
                      ...
                      <span>
                        <span className="font-bold text-foreground mr-1">{artisan.rating_average || '0.0'}</span> 
              ...
              {artisan.phone && (
                <Card className="border-border/50 bg-card rounded-3xl shadow-sm">
                  <CardContent className="p-6">
                    <h3 className="font-bold text-lg text-foreground mb-4">Contact direct</h3>
                    <div className="space-y-4">
                      <a href={`tel:${artisan.phone}`} className="flex items-center text-muted-foreground hover:text-primary transition-colors group">
                        <div className="w-10 h-10 rounded-full bg-muted group-hover:bg-primary/10 flex items-center justify-center mr-4 shrink-0 transition-colors">
                    ...
                    <div className="pt-6 mt-6 border-t border-border">
                      <WhatsAppButton 
                        phone={artisan.phone}
                        message={`Bonjour ${artisan.name}, je vous contacte depuis ArtisanCongo.`}
      ...
      <LeaveReviewModal 
        ...
        onReviewSubmitted={fetchArtisanAndReviews}
      />
    </>
  );
};

export default ArtisanPublicProfilePage;